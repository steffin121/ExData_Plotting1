...
plot4 <- function(){
  ...
  png(width = 480, height = 480, units = "px","plot4.png")
  ...
  # plot 4 code
  with(DT, plot(Voltage ~ Date, type = "l", xlab = "datetime", ylab = "Voltage", main = ""))
  ...
  legend("topright", col = c("black","#FF3513","#0433FF"), legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), lty = c(1,1), bty = "n")
  par(new=F)
  
  # plot 4 code
  with(DT, plot(Global_reactive_power ~ Date, type = "l", xlab = "datetime", ylab = "Global_reactive_power", main = "", lwd = 1))
